...
@WebServlet("/_01/loginNG.do")  
public class LoginNGServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			if (dbtype.equalsIgnoreCase("mysql")) {
			    ...
			    mb = my.checkPassword_NG(userId, password);
			} else {
			    ms = new LoginServiceSQLServerDAO(session);
			    mb = ms.checkPassword_NG(userId, password);
			}

			if (mb != null) {
				// OK, 將mb物件放入Session範圍內，識別字串為"LoginOK"
				session.setAttribute("LoginOK", mb);
				session.setAttribute("Code", "NG");
			} else {
		...
		if (errorMsgMap.isEmpty()) {
			if (requestURI != null) {
				...
				response.sendRedirect(response.encodeRedirectURL(
						request.getContextPath()+"/index_sql.jsp"));
				return;
			}
		} else {
